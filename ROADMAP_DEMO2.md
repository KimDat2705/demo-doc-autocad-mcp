...
| C | ✅ **XONG** — **Liệt kê diện tích GHI SẴN** (nhãn "… m²" nguyên văn + handle) | M | Tool `liet_ke_dien_tich_ghi_san` + `_build_stated_areas`/`self.stated_area`. Đọc mọi nhãn 'X m²' verbatim + handle + layer + cờ `co_tu_khoa_dien_tich`. KHÔNG khẳng định "sàn", KHÔNG suy hình học (0 nhãn → gợi ý đối tác cấp), KHÔNG cộng gộp (tong_hop loại riêng). Lọc mật độ `/1m2` + đuôi thập phân; probe file thật (KT 20 nhãn, KC 7.04, 9T 0, hạ tầng 9). Kiểm chứng đối kháng 2 vòng (vá bịa-đuôi-thập-phân + density-space). Test [O] 15 ca (`test_takeoff_chong_bia.py` 120/120) |
| D | ✅ **XONG** — **Tự NÊU ứng viên "X kg/bộ" / số đo** gần mã để đối tác **1-click xác nhận** | M | `tinh_dai_luong` gắn `ung_vien` vào từng `inputs_thieu[i]`. `_ung_vien_kg_moi_bo` (quét ghi chú 'X kg', loại 'TỔNG', cờ per-unit `(1 …)`/`kg/bộ` bền garble; GT inox 8.62 [67FFC]) + `_ung_vien_dim` (dim gần mã, đòi mã, loại 0.0, 'thap'+khoảng cách). Dispatch theo RESOLVER (chieu_cao cột=cao độ KHÔNG gợi vs tường gợi). Ứng viên = GỢI Ý: KHÔNG tự cắm, chỉ `inputs_bo_sung` (đối tác xác nhận) mới tính; kg KHÔNG khẳng định thuộc mã. Kiểm chứng đối kháng 27 probe = 0 lỗ. Test [P] 14 ca (134/134) |
...
| F | ✅ **XONG** — **Ước chiều cao cột theo cao độ** ("cột X cao 1 tầng") ở luồng tính lẻ | M | `_rs_chieu_cao_cot` ước = `typical_floor_h`×1000 khi đối tác không cấp; cờ `gia_dinh_cao_tang` + nguồn `suy_tu_cao_do` + `chua_chac`; `_la_cot` (nhãn thắng prefix) chặn không-cột. MÓNG resolver RIÊNG `_rs_chieu_cao_mong` (luôn hỏi). ghi_chu tách 'GIẢ ĐỊNH 1 tầng' khỏi 'GÁN VỊ TRÍ'. Không levels → hỏi. Kiểm chứng đối kháng 29 probe = 0 lỗ (F1-F7). ⚠ đổi hành vi (cập nhật test B). Test [Q] 14 ca (149/149) |
