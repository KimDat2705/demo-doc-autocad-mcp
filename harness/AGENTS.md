...
2. **C — RAM / DUNG LƯỢNG UPLOAD FILE:** chịu tải file lớn (nâng RAM Render, hạ render cap, size-guard, file-TTL). Ràng buộc là RAM cloud, không phải logic. **🕒 HOÃN TỚI CUỐI DỰ ÁN (user chốt 2026-07-26):** nâng RAM = tốn tiền túi → chờ dự án xong + gọi tài trợ. ĐỪNG tự khởi động lại (kể cả "vá code chịu tải") khi user chưa yêu cầu; demo dùng file ≤45MB. Nghiên cứu đã có: `[[project-chiu-tai-va-chi-phi]]` (READFILE=120, MAX_SESSIONS=1 nhưng cap "mềm" cần vá code, HELD `f025ad7` buggy).
3. **D — KHÁC:** vận hành/robustness, AI tự học (P-series), kiểm thử/corpus, UI, bảo mật/repo. **⭐ PIVOT AI-tự-học (2026-07-26):** BỎ kênh đối-tác-dạy-mở + P5 auto-codify + F-B web-teaching → thay bằng **KHO KIẾN THỨC DEV-SOẠN + confirm-only** (đối tác chỉ bấm xác nhận, không dạy). Kho kiến thức thuộc nhóm A. Xem `[[project-ai-tu-hoc-ke-hoach]]`.
