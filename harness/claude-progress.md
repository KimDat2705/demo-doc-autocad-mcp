...
---
## Session 2026-08-01 (nối) — 🔎 NHÓM A: **A3 LIVE** (bớt 11.597 hit ảo) + **3 NO_GO có số** + sửa 2 nhãn "done" SAI
> **CHỐT SỔ:** HEAD == origin, tree SẠCH. check.sh **[46/46] PASS · 35 MCP tool · 0 regress** — ở **MỌI lát**, các suite CŨ giữ nguyên **TỪNG CON SỐ**; tổng ca **1.467 → 1.560** (+93), check.sh **42 → 46 bước**, MCP tool **34 → 35**.
> **LIVE verify 4 lần** (`af0c879` → `639fa6c` → `349e82a` → `a61472d`), mỗi lần đủ 4 mục: prompt `2026.07.27-kb-l3` hash `239e8b7b…` **KHÔNG đổi** · kb `e55ac112…` **KHÔNG đổi** (không lát nào chạm SYSTEM_PROMPT/kho kiến thức nên không cần A/B) · `/health` ok, `ram_mb` 135,4 · trang chủ HTTP 200 đủ 4 chuỗi frontend.
