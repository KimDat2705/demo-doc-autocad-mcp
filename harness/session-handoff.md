...
> 5. ✅ **XONG — "hạn cứng 16/10/2026" KHÔNG CÓ NGUỒN GOOGLE.** Đã tự tra tận nguồn: trang deprecations **Gemini API** ghi `gemini-2.5-flash` Shutdown = **"No shutdown date announced"** và **không hề chứa** chuỗi "October 16, 2026"; trang deprecations **Vertex AI** chỉ có mục Vertex SDK (24/06/2026), **không có mốc model nào**; gọi API thật hôm nay thì `2.5-flash` và `2.5-flash-lite` **đều SỐNG**. Nguồn thật của con số là **benchr.org** (trang tổng hợp bên thứ ba) — chính `KE_HOACH_NANG_CAP_MODEL.md` mục NGUỒN đã ghi vậy, rồi nó bị chép lên ô "BẮT BUỘC" thành sự thật. ⚠ Giả thuyết "16/10 là mốc Vertex" **cũng không kiểm chứng được**, đừng chép lại. ⇒ **GĐ4 hạ ưu tiên, không còn bị ngày ép.** NHƯNG "không có ngày công bố" ≠ an toàn: trang đó ghi rõ ngày liệt kê chỉ là **"earliest possible"**, và dự án đã thấy `2.0-flash`/`1.5-flash` chết thật khi sổ ghi còn sống ⇒ thứ bảo vệ được là **phép đo định kỳ** (`do_model_song.py`, có đối chứng âm), không phải mốc ngày trong tài liệu. Chi tiết + bảng nguồn ở đầu `KE_HOACH_NANG_CAP_MODEL.md`.

## 🏁🏁 CHỐT SỔ PHIÊN 2026-08-07 — **ĐỌC KHỐI NÀY TRƯỚC, RỒI ĐỌC XUỐNG**
> **HEAD `5566bab` · cây SẠCH · CLOUD LIVE `36ab4ba`.** ⚠⚠ **3 COMMIT CHƯA PUSH** — `032aefb` (GĐ1 nâng cấp model) · `825eacd` (A/B) · `5566bab` (chốt sổ).
> ⛔ **ĐÍNH CHÍNH — `032aefb` CÓ CODE SẢN PHẨM, không phải chỉ docs:** `mcp_bridge.py` **+100 dòng** = `_gen_fallback` **chạy-lại-từ-đầu khi đổi model** + **env-hoá** `GEMINI_TEMPERATURE`/`MAX_OUTPUT_TOKENS`/`THINKING_LEVEL`. Mặc định GIỮ Y HỆT (0 / 8192 / không truyền) nên **không đổi hành vi hôm nay**, nhưng đây LÀ code chạy — push là deploy nó. Cổng `[50/50]` xanh trên đúng cây này.
> (Bản vá chuỗi model chết `da0c993` thì ĐÃ LIVE từ trước, nằm trong `36ab4ba`.)
> **CỔNG `[50/50]` PASS · `EXIT_CODE_THẬT=0` · 1.805 ca · 0 FAIL · 37 MCP tool.** `feature_list` **93 mục** (74 done · **2 partial** · 13 deferred · 4 planned).
> ⚠ **pytest KHÔNG THỂ "0 failed"**: nó crash `ValueError: I/O operation on closed file` → **`no tests ran`**, exit 1. Không phải test đỏ — pytest không chạy nổi vì test đổi `sys.stdout` lúc import. **Cổng thật là `check.sh`.** **KHÔNG có `specs/specs.json`** (không có cả thư mục `specs/`) → sổ đặc tả là `harness/feature_list.json`.
>
> ### 🎯 VIỆC ĐANG CHỜ — theo thứ tự tôi khuyến nghị
> 1. **CHỜ USER QUYẾT: đổi `GEMINI_MODEL` trên Render** — `3.6-flash` (đúng số **164/172**, ×8 tiền, ×2,3 chậm) hay `3.5-flash-lite` (155/172, **nhanh+rẻ nhất**, bẫy tốt nhất, nhưng **bịa handle 2 ca**). Ngưỡng GO của kế hoạch ĐÃ ĐẠT cho 3.6-flash. Không tự đổi.
> 2. **CONTEXT CACHING** (đầu mục MỚI, sinh ra TỪ phép đo): token **ĐẦU VÀO chiếm ~90% chi phí** (34,9k–54,6k/câu vs 321–1.196 token ra) ⇒ đòn bẩy giảm tiền **lớn hơn mọi lựa chọn model**. Chưa nghiên cứu.
> 3. **Lát ×1000 grounding-có-đơn-vị** (`mcp_bridge`) — vẫn treo; #37 đã LIVE nên đây là thứ user từng chốt xếp ngay sau.
> 4. **GĐ4 dọn `2.5-flash`** trước **16/10/2026** (hạn cứng, còn ~10 tuần).
> 5. Đọc tay nốt **14/26 ca bẫy** chưa soi (con số bẫy 3/3/2 hiện là **CẬN DƯỚI**).
>
> ### ⚠ SỐ / MỆNH ĐỀ ĐỪNG TRÍCH LẠI
> · *"`gemini-2.0-flash` đã tắt (một nấc)"* → **CẢ HAI nấc đều chết**, đã vá.
> · *"3.6-flash ít token đầu ra hơn ~17%"* (kế hoạch) → **đo ra NGƯỢC: nhiều hơn 2,1×**.
> · *"chi phí 3.6 ≈ ×3"* → **đo thật ×8,0**.
> · Trục bẫy chấm bằng **heuristic từ-khoá** (7/10/11) → **SAI**; số đúng sau đọc tay là **3/3/2**.
> · Trục đúng-số **145/153/144** (bản đầu) → **SAI** do dấu phân cách hàng nghìn; số đúng **147/164/155**.

## 🏁 2026-08-07 — **A/B 198 CÂU × 3 MODEL XONG — GO cho `3.6-flash` theo ngưỡng đã đặt trước**
> **594 lượt hỏi thật (198 × 3), 0 DÒNG RƠI** — cơ chế chống 429 (`--nghi` mới thêm + checkpoint từng dòng) làm được việc. Lần trước `battery_results_pro25.jsonl` hỏng 127/198 dòng đúng vì thiếu cái này. Kết quả: `run05` = 2.5-flash · `run06` = 3.6-flash · `run07` = 3.5-flash-lite.
>
> ### ⚠⚠ BỘ TRÍCH HỎNG LẦN THỨ HAI TRONG PHIÊN — SUÝT CHO KẾT LUẬN NO_GO NGƯỢC
> **(a) Trục BẪY:** heuristic "có cụm từ-chối" chấm ra **7 / 10 / 11 ca không-từ-chối** ⇒ trông như *cả hai model mới đều THUA trục quyết định = NO_GO*. **ĐỌC TAY 12 ca thì lật ngược:** phần lớn ca "không từ chối" lại là **TRẢ LỜI ĐÚNG** (id 30 trả đúng 8 phòng · id 97 đọc đúng ô văng + SL · id 100 nói đúng là KHÔNG có bảng bê tông · id 65/54/50/26 đều đúng). Vi phạm THẬT sau khi đọc tay: **2.5 = 3 · 3.6 = 3 · lite = 2**.
> **(b) Trục ĐÚNG SỐ:** bộ trích so số thô nên **dấu phân cách hàng nghìn làm lệch**: kỳ vọng `8024`, 2.5 viết `8024` (khớp) còn 3.6 viết `8.024` bị chấm **LỆCH DÙ ĐÚNG**. Chuẩn hoá 2 chiều rồi đo lại: **147 → 147 · 153 → 164 · 144 → 155** (bộ trích cũ hạ oan 3.6 mất **11 ca**).
> 📌 Cả hai lần đều là *"số trông hợp lý"*. Đúng khuôn `[[feedback-kiem-bo-trich-truoc-khi-tin-so]]` và cảnh báo trong chính §2.5 của kế hoạch.
>
> ### 📊 5 TRỤC — KHÔNG GỘP THÀNH MỘT ĐIỂM
> | Trục | `2.5-flash` (đang dùng) | `3.6-flash` | `3.5-flash-lite` |
> |---|---|---|---|
> | **1. Bẫy ảo giác** (ĐỌC TAY 12 ca) | 3 vi phạm | **3 vi phạm** | **2 vi phạm** |
> | **2. Đúng số** /172 | 147 (85,5%) | **164 (95,3%)** | 155 (90,1%) |
> | **3. Recall** — ca thường trả "không có" | 34,3% | 19,8% | **18,0%** |
> | **4. Thời gian** trung vị / p90 | 3,6s / 11,2s | 8,4s / 17,1s | **2,3s / 6,0s** |
> | **5. Chi phí/câu** (từ probe riêng) | $0,0113 | **$0,0909 (×8,0)** | chưa có giá xác minh |
> | Handle BỊA bị guard bắt | 0 | 0 | **2** (id 44, 138) |
>
> **⇒ NGƯỠNG GO CỦA KẾ HOẠCH ("`3.6-flash` không thua ở trục bẫy VÀ không tụt recall") = ĐẠT:** bẫy NGANG (3-3), recall hơn **14,5 điểm**, đúng số hơn **9,8 điểm**. **GO về CHẤT LƯỢNG.** Cái phải cân là **×8 tiền và ×2,3 thời gian** — đó là quyết định kinh doanh, KHÔNG tự quyết.
> **⭐ `3.5-flash-lite` là điểm cân bằng bất ngờ:** hơn model đang dùng ở CẢ đúng số (+8 ca) lẫn recall (−16,3 điểm), **nhanh nhất**, rẻ nhất — nhưng **bịa handle 2 ca** và có ca gọi mốc ranh là *"móng M1"* rồi đọc toạ độ (id 154).
>
> ### ✅ MỤC 2.6 — ĐO LẠI HÀNG RÀO: CẢ HAI RÀNG BUỘC ĐẠT
> · **GIẾT OAN (FP) = 0/198 trên CẢ BA model** — ràng buộc **CỨNG**. Soi từng ca guard có đụng: 3.6 id153 guard rút một câu TỪ CHỐI thành câu TỪ CHỐI (không mất nghĩa); lite id44/id138 guard chỉ **THÊM** cảnh báo handle-không-khớp, không xoá gì.
> · **VÙNG MÙ = 0/198 × 3 model** ⇒ **mục tiêu của lát PA-0 ĐẠT**: xoá sạch vùng mù **117/179 hàng REFUSE** của dữ liệu cũ. Mọi lượt nay đều truy được nguyên nhân.
> ⛔ KHÔNG tính/không trích "tỉ lệ lọt" tổng (thuộc tính của BỘ SINH, không phải của hàng rào).
>
> ### ⚠ GIỚI HẠN CỦA PHÉP ĐO NÀY — ĐỪNG ĐỌC QUÁ
> Chỉ ĐỌC TAY **12/26 ca bẫy** (những ca có ≥1 model bị heuristic gắn cờ). **14 ca còn lại cả ba đều "từ chối" theo từ khoá — CHƯA đọc tay**, mà một câu có cụm từ-chối vẫn có thể chứa số bịa. Con số "3 · 3 · 2" là **cận dưới**, không phải kết luận đóng.
>
> ### ⏳ CHỜ USER QUYẾT — không tự đổi model trên Render
> `3.6-flash` (chất lượng cao nhất, ×8 tiền, ×2,3 chậm) hay `3.5-flash-lite` (rẻ + nhanh nhất, hơn hẳn model hiện tại, nhưng bịa handle)? **Đòn bẩy thật vẫn là CONTEXT CACHING** — đầu vào chiếm ~90% chi phí, làm caching thì cái giá ×8 của 3.6 co lại rất nhiều.

## 🚀 2026-08-07 — **NÂNG CẤP MODEL: GĐ1 XONG** · **GĐ2 bước 1-2 xong, SỐ LẬT GIẢ ĐỊNH CỦA KẾ HOẠCH** — ĐỌC KHỐI NÀY TRƯỚC
> **GĐ1 (offline) XONG TRỌN.** Cổng `[50/50]` PASS exit 0 · **1.795 → 1.805 ca** · 0 FAIL · diff cổng ĐÚNG 1 DÒNG (suite fallback 32→42). SDK `google-genai` **2.10.0 → 2.17.0**, cổng vẫn xanh ⇒ bản mới không phá gì.
> · **1.2 chạy-lại-từ-đầu khi đổi model** (chống *thought signature* của Gemini 3): `_gen_fallback` ném `_CanChayLaiTuDau` khi ĐÃ gọi tool, lớp bọc `tra_loi_ai` dựng lại `contents` SẠCH rồi chạy với model kế. Ca test có răng nhất là **E3**: khẳng định model kế **KHÔNG HỀ được gọi** với contents nhiễm (`calls == ['m1']`), chứ không chỉ kiểm "có ném lỗi không".
> · **1.4 env-hoá** `GEMINI_TEMPERATURE` / `GEMINI_MAX_OUTPUT_TOKENS` / `GEMINI_THINKING_LEVEL` — **mặc định GIỮ Y HỆT** (0 · 8192 · không truyền thinking_config). Env để A/B, KHÔNG để đổi ngầm.
> · ⚠ **Bản vá của tôi tự đẻ 1 lỗ, test bắt được:** lớp bọc có đường mà lần gọi CUỐI lại ném chính tín hiệu nội bộ ra ngoài → `app.py` nhận ngoại lệ lạ. Đã đổi thành vòng lặp có trần cứng + trả câu trung thực khi cạn đường đổi model.
>
> ### 📊 GĐ2 bước 1-2 — R1/R2/R3 ĐỀU CÓ CÂU TRẢ LỜI BẰNG SỐ (probe rẻ, ~20 request)
> · **R3 (`FunctionResponse` cần `id`?) → KHÔNG CẦN.** Vòng gọi tool chạy trọn trên 3.6-flash, 0 lỗi 400.
> · **R2 (`max_output_tokens` là ngân sách CHUNG) → KHÔNG KÍCH ở 8192.** Token nghĩ 636-1.078 + trả 166-337, `finish` toàn `STOP`, không MAX_TOKENS.
> · **R1 (`temperature=0` xung đột Gemini 3) → temp=0 CHẠY TỐT.** 3.6-flash t=0 và t=1 đều **4/4 đúng**; t=1 tốn thêm ~14% token. Chưa thấy dấu hiệu "looping/degraded" ⇒ **giữ được `temperature=0`** (lựa chọn chống bịa cốt lõi). Mẫu nhỏ (4 câu), cần battery mới chốt.
>
> ### ⛔ SỐ ĐO LẬT HAI GIẢ ĐỊNH CỦA CHÍNH KẾ HOẠCH
> **(a) "3.6-flash ít token đầu ra hơn ~17%" — ĐO RA NGƯỢC HẲN:** nó dùng **2,1× token ra** và **1,56× token vào** (nhiều lượt gọi model hơn: 4 vs 3).
> **(b) Chi phí không phải ×3 mà là ×8.** Đo Q1 cùng một câu, cùng một bản vẽ:
>
> | Model | token VÀO | nghĩ | trả | lượt | giây | chi phí/câu |
> |---|---|---|---|---|---|---|
> | `gemini-2.5-flash` (đang dùng) | 34.928 | 237 | 84 | 3 | 4,7s | **$0,0113** |
> | `gemini-3.6-flash` | 54.616 | 1.002 | 194 | 4 | 10,8s | **$0,0909 (×8,0)** |
> | `gemini-3.5-flash` | 36.932 | 1.157 | 220 | 3 | 10,7s | (chưa có giá xác minh) |
> | `gemini-3.5-flash-lite` | 34.859 | **0** | 263 | 3 | **2,6s** | (chưa có giá xác minh) |
>
> **🔑 PHÁT HIỆN ĐÒN BẨY THẬT: token ĐẦU VÀO chiếm ~90% chi phí** (35-55k/câu, do SYSTEM_PROMPT + 37 khai báo tool + kết quả tool tích luỹ qua các lượt). ⇒ **Muốn giảm tiền thì tối ưu CONTEXT CACHING, không phải đổi model.** Đây là đầu mục mới, chưa có trong kế hoạch.
>
> ### ⭐ ỨNG VIÊN KẾ HOẠCH ĐÃ LOẠI BẰNG LÝ LẼ, ĐO LẠI THÌ RẤT MẠNH
> Kế hoạch loại `3.5-flash-lite` vì *"nhảy thẳng 3.6 → Lite hụt quá sâu"* — đó là **lý lẽ, không phải phép đo**. Đo thật: **4/4 đúng** (2 câu đọc + 2 câu bẫy đều từ chối), **0 token suy nghĩ**, **2,5-3,1s/câu — NHANH HƠN cả model đang dùng**, token vào ngang 2.5-flash. ⇒ **phải đưa vào A/B**, không loại bằng cảm tính.
>
> ### ⏳ CHỜ USER QUYẾT — A/B 198 CÂU TỐN TIỀN THẬT
> Ước tính theo số đo: `2.5-flash` ~**$2,2** · `3.6-flash` ~**$18,0** ⇒ A/B hai model ≈ **$20**. Thêm `3.5-flash-lite` thì rẻ hơn nhiều nhưng chưa có giá xác minh. **Chưa chạy — chờ user chọn phạm vi.**
...
>
> **✅ B6 ĐẠT, 0 LỖI:** diff corpus **142 file** — `p37` lệch **0/142**, `t36` lệch **0/142**, tổng ô đọc được **7.207 = 7.207**, trạng thái khớp từng nhóm, **9/9** DXFStructureError giữ `loi_doc_file`. ⚠ **Với B4 danh sách kỳ vọng là RỖNG** (không đụng ký tự logic nào) nên mọi khác biệt đều là lỗi — 0/142 là mức chặt nhất. **Đường sản phẩm ≡ đường đọc-file PHỦ TRỌN 142 file**: 119 + **14 file to chạy lại riêng với `READFILE_MAX_MB=400` (1.186 MB, to nhất 212 MB) đều KHỚP** — không bỏ im 14 file bị cổng chặn vì chúng là các bản kết cấu LỚN NHẤT. **K1-K5 qua bridge THẬT trên 93 file kích hoạt: 0 lỗi** (đặc tả đòi ≥30). **Union mốc-mm 135 ≤ trần 143**; lệch +2 so con số 133 của đặc tả đã đo có đối chứng ⇒ **bản vá giải-mã-phông đóng góp ĐÚNG 0 mốc** (0/93 file khác biệt), chênh có từ TRƯỚC B4.
> **✅ B7 SẠCH — 0 phát hiện cần xử lý. B0-B7 XONG TRỌN.** Cổng chốt sổ đo lại cuối phiên: **`[50/50]` PASS exit 0 · 1.778 ca · 0 FAIL · GIỐNG HỆT TỪNG DÒNG lần chạy B5.**
> · **Tầng 1 (câu bịa):** ⚠ vòng đầu tôi báo SAI *"TRƯỢT 11>7"* vì so hai đại lượng khác nhau — bộ đo sinh ra số 7 chạy **23 câu × 4 file, KHÔNG có F5**. Phân xử: **P1 (đúng phạm vi) = 7 ✅**, P2 (+F5) = 9, P3 (+5 câu mới) = 11; **đối chứng `_vn=identity` cũng = 7 ⇒ B4 đóng góp +0**. Tiêu chí (9) ĐẠT.
> · **Tầng 2 (đánh vào 3 delta của bản port — bề mặt MỚI):** doc dùng lại **bất biến 10/10 tool xen giữa** · `to_unicode` sinh **0/9.139** chữ số từ chuỗi không-số · `nhan_chua` model-cấp bơm **0 neo** (đóng đúng lớp lỗi #36 từng dính với `'-600'`) · tham số thù địch không crash/rò, ≤72ms.
> · 🔴 **SỐ MỚI CHƯA TỪNG ĐO:** file KÍCH-HOẠT-MỚI (#36 im) → **31 lượt câu bịa lọt / 5 file** dưới cổng 45MB; **đặc tả trước chỉ đo 5 file đích, corpus có 89 file nhóm này**. Không phải hồi quy — là **giá của việc bật #37** mà user đã chốt chấp nhận, nhưng lớn hơn hình dung cũ ⇒ **làm lát ×1000 ngay sau, đừng để lâu**.
> **⏭ CÒN LẠI: theo đặc tả, ĐÂY là lúc được phép bàn commit/LIVE** (điều kiện "B7 sạch phát-hiện-CAO" đã đạt). **Vẫn CHƯA push — chờ user quyết.** Ngay sau khi #37 LIVE: lát **grounding-có-đơn-vị** (lỗ ×1000 ở `mcp_bridge`).
> 📌 **BÀI HỌC B7:** *"11 > 7"* trông y hệt hồi quy thật. Cứu bằng đúng một câu hỏi — **con số cũ đo trên PHẠM VI NÀO?** ⇒ so số thì bộ câu / danh sách file / mức gh phải TRÙNG KHỚP, không thì "tăng/giảm" vô nghĩa.
> 📌 **CRLF làm hỏng bộ đo LẦN THỨ 3 trong phiên** (2 lần trước: ghi file đổi CRLF→LF mà `git diff` giấu; mẫu mutation 2 dòng không khớp). Lần này: kiểm hash hàm #36 bằng one-liner đọc `'rb'` ra số LỆCH, đọc chế độ TEXT (đúng như ca G-16b) thì KHỚP. ⇒ **so hash thì phải dùng CÙNG chế độ đọc với ca test đang khoá nó.**
...
> **RED-TEAM B3 (`đủ_điều_kiện_B4 = false`):** K1-K5 PASS 75/75 cấu hình qua mcp_bridge THẬT · trần rổ neo ĐẠT (mm 55≤60 · mét 85≤90 · corpus union 135≤143) · hồi quy nền XANH. **4 CAO đang vá** (59/60 fixture cũ pass mà 4 lỗ vẫn sống = đúng tiền lệ red-team): **RB-1 bảng-vanish** (thiếu `or muc_luc` dòng 1043 — C1 mất 4 hàng a/26 ô KHÔNG VẾT cả gh60 lẫn gh200; corpus 3 bảng/48 hàng) · **B3-3 nhánh câm** khung<MIN_YS (~dòng 341 — 108 dải/20 file rơi không vết, có hàng THẬT) · **B3-1 mode-flip V-F2a** (≥2 chữ khổng lồ cùng cỡ đông hơn → đọc chữ trang trí thành tầng a) · **B3-2 ghép-x nuốt bảng kề** (2 bảng thật mép chạm trộn làm 1 — fixture; corpus chưa thấy hại thật). Việc kèm: tái tạo `v_hoiquy_kq.json` · đo lại claim V-H2c bằng rot THÔ (census cũ vòng tròn — engine ép ATTRIB rot=0, corpus có 18 ATTRIB rot=270 chưa soi) · `b4_dinh_chinh_dacta.md` (danh sách đính chính DAC_TA phải áp ở B4: NGƯỠNG_CAO không tồn tại, lưới mét 0.0-inclusive, tích luỹ phiên x1,1-x2,7, gioi_han=0 falsy→60…).
> **⚠ SỐ ĐỪNG TRÍCH LẠI:** 'gh60 trả 40/41 bảng' (đếm bảng không 1-1) · tần suất mode-flip corpus (hook census mù, chưa có số) · '13.097 số bbox khung nhỏ' (artifact bình đồ — dùng 108) · claim '919 nhãn rot=90 ngoài scope' (đo vòng tròn).
