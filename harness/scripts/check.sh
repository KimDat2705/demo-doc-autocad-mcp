...
echo "=== [1/4] Import tools_core sach + dem MCP tool ==="
...
echo "=== [2/4] Khong hardcode API key ==="
...
echo "=== [3/4] Test CHONG BIA takeoff (tat dinh, offline, khong ton API) ==="
out=$($PY tests/test_takeoff_chong_bia.py 2>&1); rc=$?
echo "$out" | tail -1
[ "$rc" -eq 0 ] || { echo "FAIL: test chong bia co FAIL"; fail=1; }

echo "=== [4/4] Test MODEL FALLBACK 429/503 (robustness H, offline, mock, khong ton API) ==="
