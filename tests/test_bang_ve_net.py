...
def main():
    try:
        ...
        # (2026-07-30: bo loc doi ten thanh _strip_neo = _strip_ten_file(_strip_kb(...)) — gop them viec loai
        #  khoa 'name' (TEN FILE do nguoi dung dat, do duoc la kenh bom neo NGOAI ban ve). Y DINH cua F1 KHONG
        #  doi: ten tool nay phai bi loai khoi tool_numbers, va call-site khong duoc gom RAW.)
        _loai_ok = bool(re.search(r'fc\.name not in \([^)]*"phat_hien_bang_ve_net"[^)]*\)', _mb_src)) \
                   and 'tool_numbers |= _collect_numbers(_strip_neo(result))' in _mb_src
